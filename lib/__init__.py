...
'''
  ...
  tex += '\\hline\n'
  tex += 'Optativa de humanidades 1 & 30 & %s \\\\\n' % (('%.2f' % ((30 / ch_unit) * 100)).replace('.', ','))
  ...
  ch_percentage = ('%.2f' % (((total_human + 30) / ch_unit) * 100)).replace('.', ',')
...
'''
    ...
    for unit in units:
      ...
      if unit['Tipo'] != 'E' and unit['Tipo'] != 'T':
        tex += '%s & %d & %d & %d & %d	&	0 \\\\	\\hline\n' % (unit['Nome'], (unit['P']/2) + unit['NP'], unit['P']/2, unit['TOTAL'], unit['NP'])
      elif unit['Tipo'] == 'E':
        tex += '%s & %d & 0 & %d & %d	&	0 \\\\	\\hline\n' % (unit['Nome'], (unit['P'] + unit['NP']), unit['TOTAL'], unit['NP'])
        extensao = unit['TOTAL']
      else:
        tex += '%s & %d & 0 & %d & %d	&	0 \\\\	\\hline\n' % (unit['Nome'], (unit['P'] + unit['NP']), unit['TOTAL'], unit['NP'])
...
def generate_curricular_units (df_required, df_opt):
  ...
  for i, unit in enumerate(units):
    ...
    tes = [ re.sub(r' \(\d+h\)', '', te)[5:] for te in tes ]
    ementa = '. '.join(tes)
